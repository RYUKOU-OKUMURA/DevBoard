...
export const TabNavigation: React.FC<TabNavigationProps> = ({
  ...
  return (
    <nav
      className="flex gap-inline-sm overflow-x-auto border-b border-[var(--border-subtle)] bg-surface-primary px-inset-sm transition-colors motion-reduce:transition-none sm:px-inset-xl"
      role="tablist"
      aria-label="ビュー切り替え"
    >
      {tabs.map((tab, index) => {
        ...
        return (
          <motion.button
            ...
            data-tab-id={tab.id}
            className={`
              relative flex shrink-0 items-center gap-inline-md rounded-sm border-b-2 text-body-sm
              ...
              transition-colors motion-reduce:transition-none ${focusRing.default} focus-visible:ring-[var(--accent-green)] focus-visible:ring-opacity-75
            `}
