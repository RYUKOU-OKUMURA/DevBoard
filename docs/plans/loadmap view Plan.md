...
- [ ] `RepoProjectStage`を追加した
- [ ] `RepoScheduleBucket`を追加した
- [ ] `RepoUserMeta`に`tracked`を追加した
- [ ] 保存バージョンを2へ更新した
- [ ] バージョン1から移行できる（`tracked`欠損時は`true`）
- [ ] 既存入力が保持される
- [ ] アカウント別保存が維持される
...
- [ ] 開発段階を選択できる
- [ ] 作業予定を選択できる
- [ ] 「進捗管理に追加 / から外す」で`tracked`を切り替えられる
- [ ] 自動保存される
- [ ] 未入力時に初期値を表示できる
- [ ] 保存エラーを表示できる
...
- [ ] 表示モードを切り替えられる（すべて・カンバン・ロードマップ）
- [ ] 表示モードが保存される
- [ ] 「進捗管理に追加 / から外す」で`tracked`が切り替わる
- [ ] カンバン・ロードマップに`tracked`のみ表示される
- [ ] 外したリポジトリのメタデータが保持され、再追加で復元される
- [ ] 検索が両方へ反映される
- [ ] 並び替えが両方へ反映される
- [ ] GitHub活動状況が維持される
- [ ] Public / Private表示が維持される
- [ ] Archived表示が維持される
- [ ] キーボード操作を確認した
- [ ] スマートフォン幅を確認した
- [ ] 100件程度で動作確認した
